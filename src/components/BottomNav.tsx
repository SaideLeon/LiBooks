...
import { Screen } from '@/lib/definitions';
...
const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 w-full border-t border-gray-200/80 bg-background-light/80 backdrop-blur-lg dark:border-gray-800/80 dark:bg-background-dark/80 lg:hidden">
      <div className="flex h-20 items-center justify-around overflow-x-auto [-ms-scrollbar-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {navItems.map((item) => (
          <NavItem
            key={item.id}
            label={item.label}
