...
const NewPublicationScreen: React.FC<NewPublicationScreenProps> = ({ goBack, currentUser }) => {
  ...
  const handlePublish = async () => {
    if(!content.trim() && !image) {
        toast({
            ...
            description: "Escreva uma reflexão ou adicione uma imagem.",
        });
        return;
    }
    ...
    try {
        ...
        toast({
            title: "Sucesso!",
            description: "Sua publicação foi criada.",
        });
        goBack(); // Navigate back after successful publication
    } catch (error) {
        toast({
            variant: "destructive",
    ...
    <div className="flex h-dvh flex-col bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="flex items-start gap-3 p-4">
          <img className="mt-3 size-10 shrink-0 rounded-full object-cover" src={currentUser.avatarUrl!} alt="User avatar" />
