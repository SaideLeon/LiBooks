...
const BookDetailScreen: React.FC<BookDetailScreenProps> = ({ book, goBack, navigate }) => {
  ...
  const handleReadButtonClick = () => {
    if (progress) {
      navigate('reader', { bookId: book.id, chapterId: progress.chapterId, paragraph: progress.paragraphIndex });
    } else if (book.chapters && book.chapters.length > 0) {
      navigate('reader', { bookId: book.id, chapterId: book.chapters[0].id, paragraph: 1 });
    }
  };
