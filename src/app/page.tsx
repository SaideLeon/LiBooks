'use client';
import React, { useState, useCallback, useEffect } from 'react';
import HomeScreen from '@/screens/HomeScreen';
import BookDetailScreen from '@/screens/BookDetailScreen';
import LandingScreen from '@/screens/LandingScreen';
import ReaderScreen from '@/screens/ReaderScreen';
import SearchScreen from '@/screens/SearchScreen';
import ProfileScreen from '@/screens/ProfileScreen';
import SettingsScreen from '@/screens/SettingsScreen';
import CommunityScreen from '@/screens/CommunityScreen';
import FollowingScreen from '@/screens/FollowingScreen';
import NewPublicationScreen from '@/screens/NewPublicationScreen';
import CommentsScreen from '@/screens/CommentsScreen';
import BottomNav from '@/components/BottomNav';
import SideNav from '@/components/SideNav';
import LibraryScreen from '@/screens/LibraryScreen';
import { useUser } from '@/hooks/use-user';
import { Spinner } from '@/components/Spinner';
import { getBookById, getUserById } from '@/lib/actions';
import type { Book, User } from '@/lib/prisma/definitions';
import NewBookScreen from '@/screens/NewBookScreen';


type Screen = 'home' | 'search' | 'library' | 'profile' | 'community' | 'settings';

interface NavigationState {
  page: string;
  params?: any;
}

export default function App() {
  const { user, isLoading, login } = useUser();
  const [navigationStack, setNavigationStack] = useState<NavigationState[]>([{ page: 'home' }]);

  const navigate = useCallback((page: string, params?: any) => {
    setNavigationStack(prev => [...prev, { page, params }]);
  }, []);

  const goBack = useCallback(() => {
    if (navigationStack.length > 1) {
      setNavigationStack(prev => prev.slice(0, -1));
    }
  }, [navigationStack.length]);

  const changeTab = useCallback((tab: Screen) => {
    setNavigationStack([{ page: tab }]);
  }, []);
  
  const currentNavigationState = navigationStack[navigationStack.length - 1];
  const currentPage = currentNavigationState.page;
  
  const [currentBook, setCurrentBook] = useState<Book | null>(null);
  const [currentProfile, setCurrentProfile] = useState<User | null>(null);

  useEffect(() => {
    const fetchScreenData = async () => {
        if (currentPage === 'bookDetail' || currentPage === 'reader' || currentPage === 'editBook') {
            const book = await getBookById(currentNavigationState.params.bookId);
            setCurrentBook(book as Book);
        } else if (currentPage === 'profile') {
            if (currentNavigationState.params?.userId) {
                const profileUser = await getUserById(currentNavigationState.params.userId);
                setCurrentProfile(profileUser);
            } else if (user) {
                setCurrentProfile(user);
            }
        }
    };
    fetchScreenData();
  }, [currentPage, currentNavigationState.params, user]);


  if (isLoading) {
    return <div className="h-dvh w-screen flex items-center justify-center bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark"><Spinner /></div>;
  }
  
  if (!user) {
    return <LandingScreen onLoginSuccess={login} />;
  }

  const renderScreen = () => {
    switch (currentPage) {
      case 'home':
        return <HomeScreen navigate={navigate} currentUser={user} />;
      case 'search':
        return <SearchScreen navigate={navigate} />;
      case 'library':
        return <LibraryScreen navigate={navigate} />;
      case 'profile':
        return currentProfile ? <ProfileScreen navigate={navigate} user={currentProfile} /> : <div className="flex justify-center p-8"><Spinner/></div>;
      case 'community':
        return <CommunityScreen navigate={navigate} currentUser={user}/>;
      case 'settings':
        return <SettingsScreen />;
      case 'bookDetail':
        return currentBook ? <BookDetailScreen book={currentBook} goBack={goBack} navigate={navigate} /> : <div className="flex justify-center p-8"><Spinner/></div>;
      case 'reader':
        return currentBook ? <ReaderScreen book={currentBook} chapterId={currentNavigationState.params.chapterId} paragraph={currentNavigationState.params.paragraph} goBack={goBack} /> : <div className="flex justify-center p-8"><Spinner/></div>;
      case 'following':
        return <FollowingScreen goBack={goBack} navigate={navigate} />;
      case 'newPublication':
        return <NewPublicationScreen goBack={goBack} currentUser={user} />;
       case 'newBook':
        return <NewBookScreen goBack={goBack} navigate={navigate} />;
      case 'editBook':
        return currentBook ? <NewBookScreen goBack={goBack} navigate={navigate} existingBook={currentBook} /> : <div className="flex justify-center p-8"><Spinner/></div>
      case 'comments':
        return <CommentsScreen goBack={goBack} navigate={navigate} post={currentNavigationState.params.post} currentUser={user} />;
      default:
        return <HomeScreen navigate={navigate} currentUser={user} />;
    }
  };
  
  const getActiveTab = (): Screen => {
    const rootPage = navigationStack[0].page;
    if (['home', 'search', 'library', 'profile', 'community', 'settings'].includes(rootPage)) {
        return rootPage as Screen;
    }
    return 'home';
  }
  
  const activeTab = getActiveTab();

  const bottomNavScreens = ['home', 'search', 'profile', 'community', 'library'];
  const showBottomNav = bottomNavScreens.includes(currentPage);
  
  const CommunityFab = () => (
    <div className="fixed bottom-0 left-0 right-0 z-20 mx-auto max-w-lg pointer-events-none h-dvh lg:hidden">
      <button onClick={() => navigate('newPublication')} className="absolute bottom-24 right-4 flex h-14 w-14 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-primary shadow-lg transition-transform hover:scale-105 active:scale-95 pointer-events-auto">
        <span className="material-symbols-outlined text-3xl text-white">add</span>
      </button>
    </div>
  );

  return (
    <div className="h-dvh w-screen bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark">
      <div className="mx-auto flex h-dvh max-w-screen-2xl">
        <aside className="hidden lg:flex lg:w-64 xl:w-72 shrink-0 flex-col border-r border-zinc-200 dark:border-zinc-800">
          <SideNav navigate={navigate} changeTab={changeTab} activeTab={activeTab} currentUser={user} />
        </aside>
        
        <div className="flex flex-1 min-w-0">
          <div className="flex flex-col h-dvh flex-1">
            <main className={`flex-grow overflow-y-auto ${showBottomNav ? 'pb-20' : ''} lg:pb-0`}>
              {renderScreen()}
            </main>
            {currentPage === 'community' && <CommunityFab />}
            <div className="lg:hidden">
              {showBottomNav && <BottomNav activeTab={activeTab} setActiveTab={changeTab} />}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
